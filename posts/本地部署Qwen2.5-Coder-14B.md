...
- 初始目标：部署 `Qwen3-Coder-30B`。
- 现实情况：截至 2025 年 10 月 26 日，该模型**未开源**，无法本地运行。
- 备选方案：尝试 `Qwen2-Coder-30B-GGUF`，但经核实，Hugging Face 和 HF-Mirror 上**无此模型**（TheBloke 未发布 30B Coder 版本）。
- 最终选择：使用真实存在且性能优秀的替代模型  
...
## 3.准备运行环境
下载预编译的  ```llama.cpp``` （Windows + CUDA 12.4 版本）： ```llama-b6840-bin-win-cuda-12.4-x64.zip``` 解压到目录：
```
C:\llama\
```
在 C:\llama\ 中创建文件夹：
...
将模型文件复制到：
```
C:\llama\models\qwen2.5-coder-14b-instruct-q4_k_m.gguf
```
